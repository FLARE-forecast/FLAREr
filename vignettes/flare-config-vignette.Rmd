...
-   **openmeteo_api:** the name of the openmeteo api to use (leave empty
    if not using openmeteo); `seasonal`, `ensemble_forecast`,
    `historical`, `climate`
-   **openmeteo_model:** name of the openmeteo model to use (leave empty
    if not using openmeteo); see <https://open-meteo.com/en/docs> for
    list of models
...
-   **forecasts:**
    -   **endpoint:** s3 endpoint of forecast netcdf restart files
    -   **bucket:** s3 bucket of forecast netcdf restart files
-   **forecasts_parquet:**
