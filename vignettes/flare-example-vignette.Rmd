...
file.copy(system.file("extdata", "configuration", "default", "non_vertical_noise_config.csv", package = "FLAREr"), file.path(lake_directory, "configuration", "default", "non_vertical_noise_config.csv"))
```
...
> **Note:** `da_method` also accepts other algorithms (`etkf`, `letkf`,
> `esmda`, `pf`), but only the Ensemble Kalman Filter (`enkf`, the default)
> has been extensively tested. The other methods are **experimental** —
> use them with caution and verify results carefully.
