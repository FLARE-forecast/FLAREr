...
* `start_day`: The date that your simulation starts. Uses the YYYY-MM-DD
   format (e.g., "2019-09-20"). `2018-07-12` is the first day in the SCC project
   (when the catwalk temperature data comes online)
* `start_time`: The time of day you want to start a forecast. Because GLM
  is a daily timestep model, the simulation will start at this time. It 
  uses `mm:hh:ss` format and must only be a whole hour. It is in the UTC time. It can be any hour if only doing data 
  ...
  * 12:00:00 GMT
  * 18:00:00 GMT
* `forecast_start_day`: The date that you want forecasting to start in 
   your simulation. Uses the YYYY-MM-DD format (e.g., "2019-09-20"). The 
   difference between `start_time` and `forecast_start_day` 
