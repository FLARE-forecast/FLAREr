...
working_directory <- file.path(forecast_location, "working_directory")
```
...
```{r}
head(read_csv("forecast_location/input_data/observations_postQAQC_long.csv", col_types = readr::cols()))
```

Second, FLARE are requires the observed (historical) meteorology to be a specific name (observed-met_fcre.nc) and format. 

```{r}
ncdf4::nc_open("forecast_location/input_data/observed-met_fcre.nc")
```
Third, FLARE are requires the observed (historical) inflow and outflow to be a specific name (inflow_postQAQC.csv) and format. 

```{r}
head(read_csv("forecast_location/input_data/inflow_postQAQC.csv", col_types = readr::cols()))
```
...
```{r}
all_files <- list.files("forecast_location/input_data/FLOWS-NOAAGEFS-AR1", full.names = TRUE)
...
Similarily, the forecasted NOAA GEFS meterology has a specific file formats and file names with specific characters (NOAAGEGS_1hr). The follow set of files are the forecasted meterology files in the example.

```{r}
all_files <- list.files("forecast_location/input_data/NOAAGEFS_1hr-debias", full.names = TRUE)
all_files
```
```{r}
...
config$run_config$forecast_location <- file.path(forecast_location, "input_data")
...
To run your first FLARE simulation confirm that the following variables are set in your `run_configuration.yml` file:
...
You will find that a `working_directory` directory is created in your `forecast_location` directory. This is were all the files for the simulation are stored. After the simulation is finished all the important files are moved to the `forecast_location` directory so it is fine to delete the `working_directory`.
