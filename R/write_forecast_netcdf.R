...
write_forecast_netcdf <- function(enkf_output,
  ...
  internal_moodel_depths_dim <- ncdf4::ncdim_def("internal_moodel_depths_dim",units = '', vals = seq(1, dim(model_internal_depths)[2]), longname = 'number of possible depths that are simulated in GLM')
  ...
  def_list[[7]] <- ncdf4::ncvar_def("model_internal_depths","meter",list(timedim, internal_moodel_depths_dim, ensdim),fillvalue,longname = "depths simulated by glm that are required to restart ",prec="single")
