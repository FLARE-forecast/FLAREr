...
write_forecast_csv <- function(da_forecast_output,
  ...
  output_list <- purrr::map_dfr(1:nrow(indexes), function(i, indexes){
    ...
    var2 <- indexes$Var2[i]
    var3 <- indexes$Var3[i]
    tibble::tibble(prediction = x[var1, var2, var3, ],
                   datetime  = full_time[var1],
  ...
  for(s in 1:length(obs_config$state_names_obs)){
    ...
    output_list_tmp <- purrr::map_dfr(1:nrow(indexes), function(i, indexes){
      var1 <- indexes$Var1[i]
      var3 <- indexes$Var2[i]
      tibble::tibble(prediction = temp_var[var1, var3, ],
                     datetime  = full_time[var1],
                     depth = config$model_settings$modeled_depths[var3],
  ...
  if(length(config$output_settings$diagnostics_names) > 0){
    ...
    output_list2 <- purrr::map_dfr(1:nrow(indexes), function(i, indexes){
      ...
      var2 <- indexes$Var2[i]
      var3 <- indexes$Var3[i]
      tibble::tibble(prediction = diagnostics[var1, var2, var3, ],
                     datetime  = full_time[var1],
                     depth = config$model_settings$modeled_depths[var3],
  ...
  if(!is.null(pars)){
    ...
    output_list3 <- purrr::map_dfr(1:nrow(indexes), function(i, indexes){
      var1 <- indexes$Var1[i]
      var2 <- indexes$Var2[i]
      tibble::tibble(prediction = pars[var1, var2, ],
                     datetime  = full_time[var1],
                     depth = NA,
  ...
  for(i in 1:dim(lake_depth)[1]){
    tmp <- tibble::tibble(prediction = lake_depth[i, ],
                          datetime  = full_time[i],
                          variable = "depth",
                          depth = NA,
                          forecast = forecast_flag[i],
                          parameter = 1:dim(lake_depth)[2],
                          variable_type = "state")
  ...
  if(length(config$output_settings$diagnostics_names) > 0){
    for(i in 1:dim(diagnostics)[2]){
      tmp <- tibble::tibble(prediction = 1.7 / diagnostics[1, i, which.min(abs(config$model_settings$modeled_depths-1.0)), ],
                            datetime = full_time[i],
                            variable = "secchi",
                            depth = NA,
                            forecast = forecast_flag[i],
                            parameter = 1:dim(diagnostics)[4],
                            variable_type = "state")
      output_list <- dplyr::bind_rows(output_list, tmp)
    }
  }
  ...
  for(i in 1:dim(snow_ice_thickness)[1]){
    tmp <- tibble::tibble(prediction = apply(snow_ice_thickness[2:3, i, ], 2, sum),
                          datetime = full_time[i],
                          variable = "ice_thickness",
                          depth = NA,
                          forecast = forecast_flag[i],
                          parameter = 1:dim(snow_ice_thickness)[3],
                          variable_type = "state")
    output_list <- dplyr::bind_rows(output_list, tmp)
  }
  ...
  output_list <- output_list %>%
    ...
    dplyr::select(reference_datetime, pub_time, model_id, site_id, depth, datetime, family, parameter, variable, prediction, forecast, variable_type)
